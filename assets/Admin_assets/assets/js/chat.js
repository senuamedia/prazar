$(document).ready(function() {
	...
	if(document.getElementById("pg")){
		...
		send_btn.on("click",function(){
			...
			socket.emit("send_msg",{reciver_id:reciver_id,user_id:user_id,message:message.val(),conversation_jd:conversation_jd});
			
			var new_msg = '<div class="chat"><div class="chat-avatar"></div><div class="chat-body"><div class="chat-content"><p>'+message.val()+'</p></div></div></div>';
		...
		socket.on("success_send_img",function(data){console.log(data);
			if(data.status == 200){
				...
				if(data.message_type == 1){
					...
					var msgg='<a href="'+base_url+data.file_path+'" target="_blank"><img src="/'+data.file_path+'"></img></a>';
					
				}else{
					
					var msgg='<a style="color:#fff" href="'+base_url+'messages/'+data.file_path+'" target="_blank">'+data.message+'</a>';
					
				}
				
				var new_msg='<div class="chat"><div class="chat-avatar"></div><div class="chat-body"><div class="chat-content">'+msgg+'</div></div></div>';
		...
		socket.on("recive_message",function(data){console.log(data);
			...
			if($(".content_chat").attr("conversation_id") == conversation_jd){
				...
				if(message_type==0){
					...
					var msgg='<a href="'+base_url+file_path+'" target="_blank"><img src="'+file_path+'"><img src="/'+file_path+'"></img></a>';
				}else{
					var msgg='<a href="'+base_url+'messages/'+file_path+'" target="_blank">'+message+'</a>';
				}
				
				var new_msg='<div class="chat chat-left"><div class="chat-avatar"></div><div class="chat-body"><div class="chat-content">'+msgg+'</div></div></div>';
				...
				$("#conversation_box div").each(function(){
